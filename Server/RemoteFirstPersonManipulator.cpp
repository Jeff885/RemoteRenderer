...
const float RemoteFirstPersonManipulator::step_rotation=(osg::PI/2800);//5度
const float RemoteFirstPersonManipulator::pred_var=5;
RemoteFirstPersonManipulator::RemoteFirstPersonManipulator(Camera c):lastcoordinate_x(-1),
	lastcoordinate_y(1),
	m_distance(0),
	v_distance(0),
	m_rotation(0),
...
osg::Matrixd RemoteFirstPersonManipulator::getPredInverseMatrix(double delta_time)//预测
{
	 ...
	 if(pre_eye>0)
	 {
		eye += rotation * osg::Vec3d( 0., 0., -pred_var*step);//跟rotation相关
	 }else
	 {
...
void RemoteFirstPersonManipulator::handleUpdate(TouchEvent &touch_event)
{
	handle(touch_event._x1,touch_event._y1);
}
osg::Matrixd RemoteFirstPersonManipulator::getInverseViewMatrix()
{
	return getInverseMatrix();
}
void RemoteFirstPersonManipulator::resetPrediction()
{
}
RemoteFirstPersonManipulator::~RemoteFirstPersonManipulator()
{
	cout<<"执行RemoteFirstPersonManipulator析构"<<endl;
